	...
	if (is_super_admin()) {
		...
		if ($pay_ask == 1) {
			...
			$wp_admin_bar->add_menu( array(
				'parent' => 0,
				'id' => 'new_payments',
				'title' => '<span class="ab-icon dashicons-before dashicons-cart"></span><span class=" count-'.$count_posts_by_type.'"><span class="">'.$new_payments.'</span></span>' ,
            ...
            if (get_post_type() != 'post') {
                ...
                if (get_post_type() == 'question') {
                	...
                	if (isset($question_category[0])) {
                		echo ask_get_taxonomy_parents($question_category[0]->term_id,'question-category',TRUE,$delimiter);
                	}
...
function ask_get_taxonomy_parents($id,$taxonomy = 'category',$link = false,$separator = '/',$nicename = false,$visited = array()) {
	$chain = '';
	...
	if ($parent->parent && ($parent->parent != $parent->term_id) && !in_array($parent->parent,$visited)) {
		...
		$chain .= ask_get_taxonomy_parents($parent->parent,$taxonomy,$link,$separator,$nicename,$visited);
	}
	if ($link) {
		$chain .= '<a href="'.esc_url(get_term_link($parent,$taxonomy)).'" title="'.esc_attr(sprintf(__("View all posts in %s","vbegy"),$parent->name)).'">'.$name.'</a>'.$separator;
	}else {
...
function count_paid_question_by_type( $user_id = "", $post_type = 'post', $post_status = "publish" ) {
	global $wpdb;
	$where = "WHERE $wpdb->posts.post_type = 'question' AND $wpdb->posts.post_status = '$post_status' AND post_author = $user_id AND ( ( wp_postmeta.meta_key = '_paid_question' AND wp_postmeta.meta_value = 'paid' ) )";
			...
			if (isset($post_re[vpanel_options][$key]) && $post_re[vpanel_options][$key] == "on") {
				if (isset($post_re[vpanel_options]["theme_pages"]) && $post_re[vpanel_options]["theme_pages"] == "on") {
					if ($key == "theme_pages") {
						...
						$post_re[vpanel_options]["paid_question"] = $paid_question_page;
						...
						$post_re[vpanel_options]["activity_log_page"] = $activity_log;
						...
						$post_re[vpanel_options]["notifications_page"] = $notifications;
						echo 3;
					}
				}else {
...
function delete_question_post() {
	...
	if ($post_author > 0) {
		askme_notifications_activities($post_author,"","","","","delete_".(get_post_type($data_id) == "question"?"question":"post"),"notifications",$data_div,(get_post_type($data_id) == "question"?"question":""));
	}
...
function delete_comment_answer() {
	...
	if ($get_comment->user_id > 0) {
		askme_notifications_activities($get_comment->user_id,"","","","","delete_".($comment_type == "question"?"answer":"comment"),"notifications",$data_div,($comment_type == "question"?"answer":"comment"));
	}
