jQuery(function() {
	...
	jQuery(".add-item.add-item-2.add-item-6").on("click",function () {
		...
		if (add_item.parent().find("#"+item_id+'_'+select_val).length) {
			add_item.parent().find("#"+item_id+'_'+select_val).addClass("removered").slideUp(function() {
				jQuery(this).slideDown().removeClass("removered");
			});
		}else {
			add_item.parent().find(".category_tabs > ul").append('<li id="'+item_id+'_'+select_val+'" class="ui-state-default"><div class="widget-head ui-sortable-handle"><span>'+select_text+'</span></div><input name="'+item_name+'['+select_val+']" value="'+select_val+'" type="hidden"><a class="del-builder-item"><span class="dashicons dashicons-trash"></span></a></li>');
		}
	});
