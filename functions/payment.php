		...
		if (isset($item_transaction)) {
			...
			add_user_meta($user_id,$user_id."_payments_".$_payments,
				array(
					...
					"first_name" => esc_attr($_REQUEST['first_name']),
					"last_name" => esc_attr($_REQUEST['last_name']),
					"user_id" => $user_id,
			...
			$last_message_email = $vpanel_emails.'<img src="'.vpanel_options("logo_email_template").'" alt="">'.$vpanel_emails_2.$body.$vpanel_emails_3;
			sendEmail(esc_html($_REQUEST['payer_email']),esc_html($_REQUEST['first_name']),esc_html(get_bloginfo("admin_email")),get_bloginfo('name'),__('Instant Payment Notification - Received Payment','vbegy'),$last_message_email);
