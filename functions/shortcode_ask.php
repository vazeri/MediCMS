...
/* ask_me_select_categories */
if (!function_exists('ask_me_select_categories')) {
	function ask_me_select_categories ($rand,$select,$attr = array(),$post_id = '',$taxonomy = '') {
		return '<span class="styled-select">'.wp_dropdown_categories(array("orderby" => "name","echo" => "0","show_option_none" => __("Select a Category","vbegy"),'taxonomy' => $taxonomy, 'hide_empty' => 0,'depth' => 0,'id' => ($taxonomy == "question-category"?"question-category":"post-category").'-'.$rand,'name' => 'category','hierarchical' => true,'selected' => $select)).'</span>';
	}
}
		...
		if (!is_user_logged_in() && $pay_ask == 1) {
			...
			if (isset($_allow_to_ask) && (int)$_allow_to_ask < 1 && $pay_ask == 1 && $payment_group[$user_login_group] != 1) {
				...
				if (isset($coupons) && is_array($coupons) && $free_coupons == 1 && $active_coupons == 1) {
					foreach ($coupons as $coupons_k => $coupons_v) {
						$pay_ask_payments = $last_payments = vpanel_options("pay_ask_payment");
						if (isset($coupons_v["coupon_type"]) && $coupons_v["coupon_type"] == "percent") {
						...
						<div class="ask_question">
							<div '.(!is_user_logged_in()?"class='if_no_login'":"").'>';
								...
								if ($question_points_active == 1) {
									$out .= '<div class="alert-message info"><i class="icon-ok"></i><p><span>'.__("Note","vbegy").'</span><br>'.sprintf(__("Will lost point %s when add a new question.","vbegy"),$question_points).'</p></div>';
								}
								$out .= '
								<form class="new-question-form" method="post" enctype="multipart/form-data">
									...
									<div class="form-inputs clearfix">';
										...
										$out .= '<p>
											...
											'.ask_me_select_categories($rand_q,(int)$posted['category'],null,'','question-category').'
									...
									<div>
										...
										if ($editor_question_details == 1) {
											ob_start();
											wp_editor((isset($posted['comment'])?wp_kses_post($posted['comment']):(isset($_POST["comment"])?wp_kses_post($_POST["comment"]):"")),"question-details-".$rand_q,$settings);
											$editor_contents = ob_get_clean();
											
											$out .= '<div class="the-details the-textarea">'.$editor_contents.'</div>';
										}else {
											$out .= '<textarea name="comment" id="question-details-'.$rand_q.'" class="the-textarea" aria-required="true" cols="58" rows="8">'.(isset($posted['comment'])?$posted['comment']:(isset($_POST["comment"])?wp_kses_post($_POST["comment"]):"")).'</textarea>';
										}
										
										$out .= '<p><span class="form-description">'.__("Type the description thoroughly and in details .","vbegy").'<br>
										' . sprintf( __( '','vbegy' ), ' <code>' . allowed_tags() . '</code>' ) . '</span></p>
									</div>
					...
					<form class="new-question-form" method="post" enctype="multipart/form-data">
						...
						<div class="form-inputs clearfix">
							...
							$out .= '<p>
								...
								'.ask_me_select_categories($rand_e,(isset($posted['category'])?(int)$posted['category']:$question_category),null,$get_question,'question-category').'
								<span class="form-description">'.__("Please choose the appropriate section so easily search for your question .","vbegy").'</span>
							</p>';
						...
						<div>
							...
							$out .= '<p><span class="form-description">'.__("Type the description thoroughly and in details .","vbegy").'<br>
							' . sprintf( __( '','vbegy' ), ' <code>' . allowed_tags() . '</code>' ) . '</span></span></p>
						</div>
						...
						<form class="new-post-form" method="post" enctype="multipart/form-data">
							...
							<div class="form-inputs clearfix">';
								...
								$out .= '<p>
									...
									'.ask_me_select_categories($rand_p,(int)$posted['category'],null,'','category').'
					...
					<form class="new-post-form" method="post" enctype="multipart/form-data">
						...
						<div class="form-inputs clearfix">
							...
							$out .= '<p>
								...
								'.ask_me_select_categories($rand_e,(isset($posted['category'])?(int)$posted['category']:$category),null,$get_post,'category').'
								<span class="form-description">'.__("Please choose the appropriate section so easily search for your post .","vbegy").'</span>
							</p>';
...
function is_user_logged_in_data ($user_links = array("profile" => 1,"questions" => 1,"paid_questions" => 1,"answers" => 1,"favorite" => 1,"points" => 1,"i_follow" => 1,"followers" => 1,"posts" => 1,"follow_questions" => 1,"follow_answers" => 1,"follow_posts" => 1,"follow_comments" => 1,"edit_profile" => 1,"logout" => 1)) {
	$out = '';
	if (is_user_logged_in()) {
		...
		$out .= '<div class="row">
			...
			<div class="col-md-4">';
				...
				if (isset($user_links) && is_array($user_links) && ((isset($user_links["profile"]) && ($user_links["profile"] == 1 || $user_links["profile"] == "on")) || (isset($user_links["questions"]) && ($user_links["questions"] == 1 || $user_links["questions"] == "on")) || (isset($user_links["paid_questions"]) && ($user_links["paid_questions"] == 1 || $user_links["paid_questions"] == "on")) || (isset($user_links["answers"]) && ($user_links["answers"] == 1 || $user_links["answers"] == "on")) || (isset($user_links["favorite"]) && ($user_links["favorite"] == 1 || $user_links["favorite"] == "on")) || (isset($user_links["points"]) && ($user_links["points"] == 1 || $user_links["points"] == "on")) || (isset($user_links["i_follow"]) && ($user_links["i_follow"] == 1 || $user_links["i_follow"] == "on")) || (isset($user_links["followers"]) && ($user_links["followers"] == 1 || $user_links["followers"] == "on")) || (isset($user_links["posts"]) && ($user_links["posts"] == 1 || $user_links["posts"] == "on")) || (isset($user_links["follow_questions"]) && ($user_links["follow_questions"] == 1 || $user_links["follow_questions"] == "on")) || (isset($user_links["follow_answers"]) && ($user_links["follow_answers"] == 1 || $user_links["follow_answers"] == "on")) || (isset($user_links["follow_posts"]) && ($user_links["follow_posts"] == 1 || $user_links["follow_posts"] == "on")) || (isset($user_links["follow_comments"]) && ($user_links["follow_comments"] == 1 || $user_links["follow_comments"] == "on")) || (isset($user_links["edit_profile"]) && ($user_links["edit_profile"] == 1 || $user_links["edit_profile"] == "on")) || (isset($user_links["logout"]) && ($user_links["logout"] == 1 || $user_links["logout"] == "on")))) {
							...
							$out .= '<div class="col-md-6">
								<ul class="user_quick_links">';
									...
									if (isset($user_links) && is_array($user_links) && (isset($user_links["activity_log"]) && ($user_links["activity_log"] == 1 || $user_links["activity_log"] == "on"))) {
										$out .= '<li><a href="'.esc_url(add_query_arg(array("u" => esc_attr($user_login->ID),$get_lang_array),get_page_link(vpanel_options('activity_log_page')))).'"><i class="fa fa-thumb-tack"></i>'.__("Activity log","vbegy").'</a></li>';
									}
...
function ask_login ($atts, $content = null) {
	...
	if (is_user_logged_in()) :
		...
		$out .= do_action('oa_social_login').(shortcode_exists('wordpress_social_login')?'<div class="clearfix"></div><br>'.do_shortcode("[wordpress_social_login]"):"").(shortcode_exists('apsl-login-lite')?'<div class="clearfix"></div><br>'.do_shortcode("[apsl-login-lite]"):"").'<div class="ask_form inputs">
...
function ask_registration_save ($user_id) {
	$point_new_user = vpanel_options("point_new_user");
	$active_points = vpanel_options("active_points");
	if (is_admin() && $point_new_user > 0 && $active_points == 1) {
		$current_user = get_user_by("id",$user_id);
		$_points = get_user_meta($user_id,$current_user->user_login."_points",true);
		$_points++;
