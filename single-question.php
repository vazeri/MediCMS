				...
				if ($question_control_style == "style_1" && (($post->post_author != 0 && $user_login_id_l->ID == $user_get_current_user_id) || ($question_follow == 1 && is_user_logged_in()) || (($post->post_author != 0 && $user_login_id_l->ID == $user_get_current_user_id) || is_super_admin($user_get_current_user_id)))) {?>
					<div class="edit-delete-follow-close">
						<h2>
							...
							if ($question_follow == 1 && is_user_logged_in() && get_current_user_id() != $post->post_author) {?>
				...
				<div class="question-inner">
					...
					<div class="question-desc" itemprop="mainContentOfPage">
						...
						if ($custom_permission != 1 || (is_user_logged_in() && isset($roles["show_question"]) && $roles["show_question"] == 1) || (!is_user_logged_in() && $show_question == 1) || $user_get_current_user_id == $post->post_author) {
							...
							if (is_user_logged_in()) {
								if ($user_get_current_user_id != $post->post_author) {
									...
									if (isset($_favorites[0])) {
										...
										echo '<a class="add_favorite add_favorite_in color button small margin_0" title="'.__("Add a question to Favorites","vbegy").'" href="#">'.__("Add a question to Favorites","vbegy").'</a>';
									}
								}
	...
	if ($yes_private == 1) {
		...
		foreach ($order_sections as $key_r => $value_r) {
						...
						if ($post->post_author != 0) {
							...
							<div class="about-author clearfix">
							    ...
							    <div class="author-bio">
							        <h4>
							        	<?php echo __("About","vbegy")." <a href='".vpanel_get_user_url($post->post_author,$authordata->user_nicename)."'>".get_the_author()."</a>";
			...
			<ul>
							              ...
							              <li><i class="icon-inbox"></i> <?php
									echo " <strong>Peso: </strong>" ;?><span><?php echo the_author_meta('phone')?></span></li>
									
									      <li><i class="icon-globe"></i> <?php
									echo "<strong>Pais: </strong>" ;?><span><?php echo the_author_meta('country')?></span></li>
									     
  										 <li><i class="icon-ambulance"></i> <?php
									echo "<strong>Antecedentes:</strong> " ;?><span><?php echo the_author_meta('city')?></span></li>
									
							         <li><i class="icon-stethoscope"></i> <?php
									echo "<strong>Edad: </strong> " ;?><span><?php echo the_author_meta('age')?></span></li>
																
									  <li><i class="icon-user-md"></i> <?php
									echo "<strong>Sexo: </strong>" ;?><span><?php echo the_author_meta('sex')?></span></li>
									
									 <li><i class="icon-briefcase"></i> <?php
									echo "<strong>Ocupación: </strong>" ;?><span><?php echo the_author_meta('website')?></span></li>
									
				</ul>					
					</div>
					
				<li><i class=" icon-list-alt"></i> <?php
									echo "<strong> Observaciones: </strong> " ;?><span><?php echo the_author_meta('description')?></span></li>
