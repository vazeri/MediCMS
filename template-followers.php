...
get_header();
	...
	$following_you_array = $following_you[0];
	
	if (isset($following_you_array) && is_array($following_you_array) && !empty($following_you_array)) {
		$rows_per_page = get_option("posts_per_page");
		$paged         = (get_query_var("paged") != ""?(int)get_query_var("paged"):(get_query_var("page") != ""?(int)get_query_var("page"):1));
		$offset		   = ($paged-1)*$rows_per_page;
		$users		   = get_users(array('include' => $following_you_array,'blog_id' => 1,'orderby' => 'registered'));
		$query         = get_users(array('offset' => $offset,'number' => $rows_per_page,'include' => $following_you_array,'blog_id' => 1,'orderby' => 'registered'));
		...
		echo "<div class='page-content page-content-user'><p class='no-item'>".__("There are no Followers yet .","vbegy")."</p></div>";
	}
get_footer();?>