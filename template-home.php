...
get_header();
	...
	$vbegy_tabs_type       = rwmb_meta('vbegy_tabs_type','radio',$post->ID);
	$vbegy_categories_show = rwmb_meta('vbegy_categories_show','type=questions_categories',$post->ID);
	...
	if ($vbegy_index_tabs == 1) {?>
		<div class="tabs-warp question-tab">
			<?php $pagination_tabs = array();
			if ($vbegy_pagination_tabs == 1) {
				$pagination_tabs = array("paged" => $paged);
			}
			if ($vbegy_tabs_type == "categories") {?>
			    <ul class="tabs">
			    	<?php if (isset($vbegy_categories_show) && is_array($vbegy_categories_show)) {
			    		foreach ($vbegy_categories_show as $key_cat => $value_cat) {
			    			$term_cat = get_term_by('id',$value_cat,'question-category')?>
			    			<li class="tab"><a href="#" data-js="categories_<?php echo $post->ID?>_<?php echo $value_cat?>"><?php echo ($value_cat == 0?__('Show All Categories','vbegy'):$term_cat->name)?></a></li>
			    		<?php }
					}?>
				</ul>
				<?php if (isset($vbegy_categories_show) && is_array($vbegy_categories_show)) {
						...
						<div class="tab-inner-warp">
							<div class="tab-inner">
								<?php if ($value_cat == 0) {
									...
									$cat_array = array('tax_query' => array(array('taxonomy' => 'question-category','field' => 'id','terms' => $value_cat)));
								}
								$args = array_merge($pagination_tabs,$cat_array,array("post_type" => "question","posts_per_page" => $posts_per_page));
								query_posts($args);
	...
	<?php }else if ($vbegy_index_tabs == 2) {
